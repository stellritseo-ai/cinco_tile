...
export async function connectDB() {
  ...
  if (!cached.promise) {
    ...
    cached.promise = mongoose.connect(MONGODB_URI, opts).then(async (m) => {
      ...
      try {
        const AdminUser = m.models.AdminUser || m.model("AdminUser", AdminUserSchema);
        const exists = await AdminUser.findOne({ username: "cinco" });
        if (!exists) {
          ...
          console.log("Default admin user 'cinco' created.");
        }
      } catch (err) {
...
const GalleryPhotoSchema = new mongoose.Schema({
  ...
  publicId: { type: String, default: "" }, // ADDED: Cloudinary publicId
