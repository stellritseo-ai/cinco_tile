          ...
          <nav className="flex flex-col gap-1 mt-2">
            {["HOME", "ABOUT", "SERVICES", "RESIDENTIAL", "COMMERCIAL", "PROJECTS", "CONTACT"].map((l, i) => (
              <a
                ...
                href={`#${l.toLowerCase()}`}
                onClick={() => setIsOpen(false)}
                className={`text-[16px] font-bold tracking-widest py-3.5 border-b border-white/5 transition-all duration-300 flex items-center justify-between hover:translate-x-2 group ${l === "HOME" ? "text-brand" : "text-white/80 hover:text-white"
                  }`}
